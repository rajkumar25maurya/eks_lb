...
resource "null_resource" "aws_lb_create" {
  provisioner "local-exec" {
    command = "kubectl apply -f lb-ctrl.yaml"
...
resource "null_resource" "metrics-server" {
  ...
  depends_on = [ aws_eks_node_group.nodes_general, aws_eks_cluster.eks_cluster, null_resource.kubectl, null_resource.cert_create, null_resource.aws_lb_create ]
...
resource "time_sleep" "wait" {
  depends_on = [ aws_eks_node_group.nodes_general, aws_eks_cluster.eks_cluster, null_resource.kubectl, null_resource.cert_create, null_resource.aws_lb_create, null_resource.metrics-server ]  
    create_duration = "90s"
}
